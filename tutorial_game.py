...
def isInside(sprite, mouse_x, mouse_y) -> bool:
    # grab the sprite's bounding box
    bounds = sprite.getWorldBounds()

    # check mouse position is in the sprite box
...
class MyASGEGame(pyasge.ASGEGame):
    ...
    def clickHandler(self, event: pyasge.ClickEvent) -> None:
        # look to see if mouse is clicked
    ...
    def spawn(self) -> None:
        # generate random(x,y) but dont let the fish spawn on edges
